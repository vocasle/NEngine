...
ShaderManager::CreateVertexShader(const std::string &filepath,
    ...
    m_inputLayouts[shaderName] =
        InputLayout(device, &bytes[0], bytes.size());
}
...
ShaderManager::Recompile(ID3D11Device *device) {
    ...
    do {
        ...
        if (shaderName.extension().string() != ".hlsl")
            continue;
        // compile shader
        const std::string shaderNameStr =
            UtilsFormatStr("%s/%s",
                           m_shaderSrcRootDir.c_str(),
        ...
        HR(D3DCompileFromFile(shaderPathW.c_str(), nullptr,
            D3D_COMPILE_STANDARD_FILE_INCLUDE, "main",
            isVS ? "vs_5_0" : "ps_5_0", 0, 0,
            &shaderBlob, &errorBlob))

        if (errorBlob.Get()) {
            UtilsDebugPrint(
                "ERROR: Failed to hot reload %s, because of compile error. %s\n",
                shaderName.c_str(),
                static_cast<char *>(
