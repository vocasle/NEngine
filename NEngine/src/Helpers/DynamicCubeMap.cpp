...
void
DynamicCubeMap::BuildCubeFaceCamera(const glm::vec3 &origin)
{
    // Generate the cube map about the given position.
    glm::vec3 worldUp(0.0f, 1.0f, 0.0f);
    // Look along each coordinate axis.
    glm::vec3 targets[6] = {
        glm::vec3(origin.x + 1.0f, origin.y, origin.z),  // +X
        glm::vec3(origin.x - 1.0f, origin.y, origin.z),  // -X
        glm::vec3(origin.x, origin.y + 1.0f, origin.z),  // +Y
        glm::vec3(origin.x, origin.y - 1.0f, origin.z),  // -Y
        glm::vec3(origin.x, origin.y, origin.z + 1.0f),  // +Z
        glm::vec3(origin.x, origin.y, origin.z - 1.0f)   // -Z
    };
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        m_cubeMapCamera[i].SetViewDimensions(CUBEMAP_SIZE, CUBEMAP_SIZE);
...
DynamicCubeMap::CreateRenderTargets(ID3D11Device *device)
{
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        HR(device->CreateRenderTargetView(
            cubeTex.Get(), &rtvDesc, &m_dynamicCubeMapRTV[i]));
    }
    ...
    HR(device->CreateShaderResourceView(
        cubeTex.Get(), &srvDesc, m_dynamicCubeMapSRV.ReleaseAndGetAddressOf()));
}
